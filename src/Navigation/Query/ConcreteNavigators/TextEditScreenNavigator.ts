...
import { ScreenResultPayload, ScreenResult, ScreenError, addScreenResultListener } from '../../../../modules/rnn_passback'
...
class TextEditScreenNavigator implements UrlQueryNavigator {
    readonly componentName = 'TextEditScreen'
    ...
    navigate<Result>(url: URL, props: any): Promise<Result> {
       if (!this.canNavigate(url)) { return Promise.reject(new Error('Url is unsupported by ${this.constructor.name}  "${url.toString()}"')) }
    ...
    private pendingResults:  { [key: string]: {id: string, callback: (payload: ScreenResult | ScreenError) => void} } = {};

    private waitForResult<Result>(id: string, componentId: string, resolve: (value: Result) => void, reject: (reason: any) => void): void {
    ...
    private isScreenResult(status: ScreenResult | ScreenError): status is ScreenResult {
